...
func main() {
    ...
    // 创建web3客户端
    w3client := blockchain.MustNewWeb3(rpcURL, privateKey, cmd.ProviderOption)
    defer w3client.Close()

    // 创建indexer客户端
    indexerClient, err := indexer.NewClient(indexerURL, indexer.IndexerClientOption{
        ProviderOption: cmd.ProviderOption,
        LogOption:      cmd.Common.LogOption{Logger: logrus.StandardLogger()},
    })
    if err != nil {
...
func downloadFileFromRoot(ctx context.Context, w3client *web3go.Client, indexerClient *indexer.Client, root string, outputPath string) error {
    // 从indexer获取可用的存储节点
    nodes, err := indexerClient.GetNodes(ctx)
    if err != nil {
    ...
    // 创建存储节点客户端
    clients := make([]*node.ZgsClient, 0, len(nodes))
    for _, n := range nodes {
        client, err := node.NewZgsClient(n.Url, cmd.ProviderOption)
        if err != nil {
            logrus.WithError(err).Warnf("创建节点客户端失败: %s", n.Url)
            continue
        }
    ...
    // 创建下载器
    downloader, err := transfer.NewDownloader(clients, cmd.Common.LogOption{Logger: logrus.StandardLogger()})
    if err != nil {
